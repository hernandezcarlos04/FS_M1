...
Implementar la clase LinkedList, definiendo los siguientes métodos:
  ...
  - search: recibe un parámetro y lo busca dentro de la lista, 
    con una particularidad: el parámetro puede ser un valor o un 
    callback. En el primer caso, buscamos un nodo cuyo valor coincida 
    con lo buscado; en el segundo, buscamos un nodo cuyo valor, al 
...
LinkedList.prototype.add=function(val){
  let node = new Node(val); //Se crea la instancia del nodo
  if(!this.head){
    this.head = node;
  }else{
    let ndoActual = this.head;
    
    while(ndoActual.next){
      ndoActual = ndoActual.next;
    }
    ndoActual.next = node;
  }
...
LinkedList.prototype.remove=function(){
  ...
  if(dele === null) return null;
    ...
    while(dele.next){
       if(dele.next.next === null){
          let valor = dele.next.value;
          dele.next = null;
          return valor;
      }
      dele = dele.next;
...
LinkedList.prototype.search = function(value){
  ...
  while(ndoActual){
    if(ndoActual.value === value) return ndoActual.value;
    else if(typeof value == 'function'){
      if(value(ndoActual.value)){
        return ndoActual.value;
      }
    }
    ndoActual = ndoActual.next;
  }
...
HashTable.prototype.hash = function(key){
  ...
  for(let i=0; i<key.length; i++){
    sum+=key.charCodeAt(i);//Tomando el valor de cada letra
  }
